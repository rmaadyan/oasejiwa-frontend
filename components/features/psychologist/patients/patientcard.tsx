...
export default function PatientCard({ patient, onViewDetails }: PatientCardProps) {
  return (
    ...
    >
      {/* Header */}
      <div className="mb-4">
        <h3 className="font-semibold text-[#2B5379] text-lg">{patient.name}</h3>
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="text-center p-2 bg-gray-50 rounded-lg">
          ...
          <p className="text-lg font-bold text-[#2B5379]">{patient.totalSessions}</p>
        </div>
        <div className="text-center p-2 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-600 mb-1">Sesi Pertama</p>
          <p className="text-xs font-medium text-gray-900">{patient.firstSessionDate}</p>
        </div>
        <div className="text-center p-2 bg-gray-50 rounded-lg">
          <p className="text-xs text-gray-600 mb-1">Sesi Terakhir</p>
          <p className="text-xs font-medium text-gray-900">{patient.lastSessionDate || "-"}</p>
        </div>
      </div>
      ...
      {patient.upcomingSessionDate && (
        <div className="flex items-center gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <div className="flex-1">
            <p className="text-xs text-blue-900 font-medium">Sesi Mendatang</p>
            <p className="text-xs text-blue-700">{patient.upcomingSessionDate}</p>
          </div>
        </div>
      )}
