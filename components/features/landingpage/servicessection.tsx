...
export default function ServicesSection() {
  ...
  const startAnimation = () => {
    ...
    controls.start({
      x: -totalWidth,
      transition: {
        duration: 60, // 30 detik untuk 1 loop
        ease: "linear",
        repeat: Infinity,
      },
    });
        ...
        >
          ...
          >
            {items.map((item, index) => (
              <article
                key={`${item.id}-${index}`}
                className="h-auto w-56 sm:w-64 lg:w-72 shrink-0"
              >
                {/* Card Content - Tanpa Gambar Background */}
                <div className="flex flex-col rounded-2xl bg-[#D1EAFF] px-6 py-6 h-full shadow-md hover:shadow-lg transition-shadow">
                  <h3 className="text-center text-lg font-semibold text-primary-text leading-tight">
                    {item.name}
                  </h3>

                  {/* Deskripsi - Full height, no scroll */}
                  <div className="mt-4 flex-1">
                    <p className="text-center text-sm leading-relaxed text-[#2B5379]">
                      {item.description}
                    </p>
                  </div>
