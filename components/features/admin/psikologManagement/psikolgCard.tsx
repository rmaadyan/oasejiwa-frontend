...
export default function PsikologCard({
    ...
    const displayName = psikolog.fullName || psikolog.name || '-';
    const displaySipp = psikolog.sipp || psikolog.licenseNumber || '-';
    const displayPhoto = psikolog.avatarUrl ? psikolog.avatarUrl : psikolog.photo || null;

    const MAX_NAME_LENGTH = 50;
    const shouldTruncateName = displayName.length > MAX_NAME_LENGTH;

    const MAX_SIPP_LENGTH = 20;
    const shouldTruncateSIPP = displaySipp.length > MAX_SIPP_LENGTH;
        ...
        <div className="min-h-22 relative border border-gray-300 rounded-xl py-2 px-4 bg-white shadow w-full max-w-sm hover:bg-gray-50 cursor-pointer">
            <div className="flex gap-2 pr-14">
                ...
                <div className="flex flex-col gap-1 flex-1 min-w-0 pt-4">
                    <p className={`text-secondary-heading text-sm font-medium ${shouldTruncateName ? 'truncate' : ''}`}>
                        {displayName}
                    </p>
