// components/features/manajemen-tes/DetailTesForm.tsx
...
const DEFAULT_KATEGORI: DiagnosisKategori[] = [
  {
    ...
    maxPersen: 33,
    deskripsi:
      "Tidak ada indikasi masalah yang bermakna. Pertahankan pola hidup sehat dan mekanisme koping yang sudah baik.",
  },
  {
    id: "k2",
    nama: "Sedang",
    ...
    maxPersen: 66,
    deskripsi:
      "Perlu meningkatkan keterampilan manajemen stres, relaksasi, dan dukungan sosial.",
  },
  {
    id: "k3",
    nama: "Tinggi",
    ...
    maxPersen: 100,
    deskripsi:
      "Disarankan konsultasi dengan profesional (psikolog/psikiater) untuk asesmen lebih lanjut.",
  },
];
...
export default function DetailTesForm({ initial, onSave, onCancel }: Props) {
  ...
  const [kategori, setKategori] =
    useState<DiagnosisKategori[]>(DEFAULT_KATEGORI);
  const [pertanyaan, setPertanyaan] = useState<PertanyaanItem[]>([]);
  const [sectionKategori, setSectionKategori] =
    useState<SectionKategoriMap>({});
  ...
  useEffect(() => {
    if (initial) {
      ...
      setKategori(
        initial.kategori?.length ? initial.kategori : DEFAULT_KATEGORI,
      );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const cleanedPertanyaan = pertanyaan.filter(
      (p) => p.teks.trim() !== "",
    );
  ...
  const ubahPertanyaan = (
    id: string,
    field: keyof PertanyaanItem,
    value: string,
  ) => {
    setPertanyaan((prev) =>
      prev.map((p) =>
  ...
  const ubahSectionKategori = (
    sectionName: string,
    id: string,
    field: keyof SectionKategori,
    value: string,
  ) => {
    setSectionKategori((prev) => {
          ...
          {pertanyaan.map((p, idx) => (
            ...
            >
              ...
              <textarea
                value={p.teks}
                onChange={(e) =>
                  ubahPertanyaan(p.id, "teks", e.target.value)
                }
                rows={2}
              ...
              <div className="mt-2 flex flex-wrap items-center gap-3 text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <select
                    value={p.arah}
                    onChange={(e) =>
                      ubahPertanyaan(p.id, "arah", e.target.value)
                    }
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    value={p.section ?? ""}
                    onChange={(e) =>
                      ubahPertanyaan(p.id, "section", e.target.value)
                    }
            ...
            >
              ...
              <div className="grid grid-cols-[minmax(0,1fr),80px] items-center gap-3">
                <input
                  value={l.label}
                  onChange={(e) =>
                    ubahLikert(l.id, "label", e.target.value)
                  }
                ...
                <input
                  ...
                  value={l.value}
                  onChange={(e) =>
                    ubahLikert(l.id, "value", e.target.value)
                  }
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {kategori.map((k) => (
            <div
              key={k.id}
              className="space-y-2 rounded-xl border border-gray-200 bg-[#f5f7fb] p-3"
            >
              <div className="flex items-center justify-between gap-2">
                <input
                  value={k.nama}
                  onChange={(e) =>
                    ubahKategori(k.id, "nama", e.target.value)
                  }
              ...
              <div className="flex items-center gap-2 text-xs text-gray-800">
                ...
                <input
                  ...
                  value={k.minPersen}
                  onChange={(e) =>
                    ubahKategori(k.id, "minPersen", e.target.value)
                  }
                ...
                <input
                  ...
                  value={k.maxPersen}
                  onChange={(e) =>
                    ubahKategori(k.id, "maxPersen", e.target.value)
                  }
                  className="w-16 rounded-md border border-gray-300 px-2 py-1"
                />
              </div>
              ...
              <textarea
                ...
                value={k.deskripsi ?? ""}
                onChange={(e) =>
                  ubahKategori(k.id, "deskripsi", e.target.value)
                }
              ...
              <textarea
                ...
                value={k.result ?? ""}
                onChange={(e) =>
                  ubahKategori(k.id, "result", e.target.value)
                }
                className="mt-1 w-full rounded-md border border-gray-300 px-2 py-1 text-xs text-gray-800"
      ...
      <div className="rounded-xl bg-white p-4 shadow-sm">
        ...
        {Object.keys(sectionKategori).length === 0 ? (
          <p className="text-[11px] text-gray-500">
            Belum ada kategori per dimensi. Bagian ini bisa dikosongkan jika
            tidak diperlukan (tes non-DASS).
          </p>
        ) : (
          <div className="mt-2 space-y-4">
            {Object.entries(sectionKategori).map(([sectionName, list]) => (
              <div
                key={sectionName}
                className="rounded-xl border border-gray-200 bg-[#f5f7fb] p-3"
              >
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-xs font-semibold text-gray-800">
                    Dimensi: {sectionName}
                ...
                {list.length === 0 ? (
                  <p className="text-[11px] text-gray-500">
                    Belum ada kategori untuk dimensi ini.
                  </p>
                ) : (
                  <div className="space-y-2">
                    {list.map((k) => (
                      <div
                        key={k.id}
                        className="grid grid-cols-[minmax(0,1.3fr),70px,70px,minmax(0,1.7fr),32px] items-start gap-2 rounded-lg border border-gray-200 bg-white p-2"
                      >
                        <input
                          value={k.nama}
                          onChange={(e) =>
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            hapusSectionKategoriItem(sectionName, k.id)
                          }
                          className="inline-flex h-7 w-7 items-center justify-center rounded-full border border-red-200 bg-red-50 text-red-500 hover:bg-red-100"
                          aria-label="Hapus kategori dimensi"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
