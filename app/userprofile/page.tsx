...
export default function Profile() {
  ...
  // 🟢 Default tab diset ke "dashboard" agar tampilan modern langsung muncul
  const [activeTab, setActiveTab] = useState<"dashboard" | "profile" | "bookings" | "tes">("dashboard");
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex items-center justify-center font-poppins">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-[#234463] font-semibold text-sm">Memuat dashboard...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#E8F3FC] font-poppins pt-20 lg:pt-24 pb-16 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        {/* Header Title */}
        <div className="flex items-center pb-6 relative justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#234463] tracking-wide">
              {activeTab === "dashboard" ? "Dashboard Pasien" : activeTab === "profile" ? "My Profile" : activeTab === "bookings" ? "Jadwal Konseling Saya" : "Riwayat Tes Psikologi"}
            </h1> 
            <p className="text-xs sm:text-sm text-slate-500 mt-1">
              Selamat datang kembali, <b>{profileData.fullName || "Pengguna Oase Jiwa"}</b>
            </p>
          </div>

          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="lg:hidden p-2.5 rounded-xl bg-white border border-blue-200 shadow-sm hover:bg-blue-50 transition"
          >
            {isSidebarOpen ? <X size={22} className="text-[#234463]" /> : <Menu size={22} className="text-[#234463]" />}
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
          ...
          <main className="flex-1 w-full">
            ...
            {/* 🟢 1. TAMPILAN DASHBOARD USER MODERN */}
            {activeTab === "dashboard" ? (
              <div className="space-y-6">
                
                {/* Widget Quote & Progress Bar */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-white border border-blue-100 rounded-2xl p-6 shadow-sm space-y-4">
                    <h3 className="font-bold text-[#234463] text-base flex items-center gap-2">
                      <HeartHandshake size={18} /> Butuh Teman Cerita?
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Jadwalkan sesi konseling dengan psikolog klinis profesional Oase Jiwa secara online maupun tatap muka langsung.
                    </p>
                    <Link
                      href="/layanan"
                      className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#234463] text-white rounded-xl text-xs font-semibold hover:bg-[#2B5379] transition shadow-xs"
                    >
                  ...
                  <div className="bg-white border border-blue-100 rounded-2xl p-6 shadow-sm space-y-4">
                    <h3 className="font-bold text-[#234463] text-base flex items-center gap-2">
                      <BrainCircuit size={18} /> Tes Kesehatan Mental
                    </h3>
                    <p className="text-xs text-slate-500 leading-relaxed">
                    ...
                    <Link
                      href="/tes"
                      className="inline-flex items-center gap-2 px-4 py-2.5 border border-[#234463] text-[#234463] rounded-xl text-xs font-semibold hover:bg-blue-50 transition"
                    >
