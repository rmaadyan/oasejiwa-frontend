...
export default function PreviewLayananPage() {
  ...
  // 🟢 Cek semua kemungkinan nama field gambar dari API backend
  const rawImagePath =
    layanan.coverUrl ||
    (layanan as any).imageUrl ||
    (layanan as any).image;

  // 🟢 Ubah path relatif menjadi URL absolut ke port backend NestJS
        ...
        <div className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-100">
          {rawImagePath ? (
            ...
            <img
              ...
              onError={(e) => {
                // Fallback jika file fisik di folder uploads backend terhapus
            ...
            <div className="flex h-64 w-full items-center justify-center bg-slate-100 md:h-80">
              <span className="text-sm text-slate-400">Ilustrasi layanan tidak tersedia</span>
            </div>
          )}
          ...
          <div className="grid gap-8 px-6 py-6 md:grid-cols-[2fr,1.2fr] md:px-8 md:py-8">
            ...
            <aside className="space-y-4 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-4 shadow-sm md:px-5 md:py-5">
              ...
              <button className="w-full rounded-full bg-[#1f3b5b] px-6 py-2 text-sm font-semibold text-white hover:bg-blue-900 disabled:opacity-60 cursor-pointer">
                Booking Sesi
              </button>
